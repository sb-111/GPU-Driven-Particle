...
void GP::SDFBaker::Bake(MeshSDF& out, Mesh& mesh, uint32_t resolutionX, uint32_t resolutionY, uint32_t resolutionZ)
{
	out.resolution[0] = resolutionX;
	out.resolution[1] = resolutionY;
	out.resolution[2] = resolutionZ;
	out.volume.Create(L"Mesh SDF", resolutionX, resolutionY, resolutionZ, DXGI_FORMAT_R16_FLOAT);
	...
	cpt.TransitionResource(out.volume, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, true);
	...
	cpt.SetDynamicDescriptor(3, 0, out.volume.GetUAV());
	cpt.Dispatch((resolutionX + 7) / 8, (resolutionY + 7) / 8, (resolutionZ + 7) / 8);
	cpt.TransitionResource(out.volume, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	cpt.Finish(true);
}
